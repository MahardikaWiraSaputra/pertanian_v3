...
class Umkm extends CI_Controller {
    ...
    function __construct(){
        ...
        $this->auth->cek_auth('data_umkm');
    }

    public function index()
    {
        $data = '';
        $this->load->view('umkm/index', $data);
    }
    ...
    public function list_data(){
        ...
        $data['total_items'] = $this->model_umkm->list_total($status,$like);
        $data['list_items'] = $this->model_umkm->list_data($status,$like,$limit,$offset);
        $this->load->view('umkm/v_list', $data );
    }

    // tambah
    public function tambah(){
        $data = '';
        $this->load->view('umkm/v_tambah', $data);
    }
    ...
    public function simpan() {
        ...
        if($this->form_validation->run()) {
            ...
            $query = $this->model_umkm->tambah($data);
            if ($query) {
                $output['success'] = true;
    ...
    //     if ($id) {
    //         $data['detail'] = $this->model_umkm->detail($id);
    //         $this->load->view('umkm/v_detail', $data);
    //     }
    //     else {
    ...
    public function edit($id){
        if ($id) {
            $data['detail'] = $this->model_umkm->detail($id);
            $this->load->view('umkm/v_edit', $data);
        }
        else {
            echo 'id tidak boleh kosong';
        }
    ...
    public function update() {
        ...
        if($this->form_validation->run()) {
            ...
            $query = $this->model_umkm->update($data);
            if ($query) {
                $output['success'] = true;
                $output['message'] = 'DATA BERHASIL DISIMPAN';
            }
    ...
    public function delete($id){
        if($id) {         
            $query = $this->model_umkm->delete($id);
            if ($query) {
                $output['success'] = true;
