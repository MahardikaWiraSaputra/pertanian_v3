...
class Api_key extends CI_Controller {
    ...
    function __construct(){
        parent::__construct();
        $this->load->model('model_api_key');
        $this->auth->cek_auth('data_api_key');
    }

    public function index()
    {
        $data = '';
        $this->load->view('api_key/index', $data);
    }
    ...
    public function list(){
        ...
        $data['total_items'] = $this->model_api_key->list_total($status,$like);
        $data['list_items'] = $this->model_api_key->list_data($status,$like,$limit,$offset);
        $this->load->view('api_key/v_list', $data );
    }

    // tambah
    public function tambah(){
        $data = '';
        $this->load->view('api_key/v_tambah', $data);
    }
    ...
    public function simpan() {
        ...
        if($this->form_validation->run()) {
            ...
            $query = $this->model_api_key->tambah($data);
            if ($query) {
                $output['success'] = true;
    ...
    //     if ($id) {
    //         $data['detail'] = $this->model_api_key->detail($id);
    //         $this->load->view('api_key/v_detail', $data);
    //     }
    //     else {
    ...
    public function edit($id){
        if ($id) {
            $data['detail'] = $this->model_api_key->detail($id);
            $this->load->view('api_key/v_edit', $data);
        }
        else {
            echo 'id tidak boleh kosong';
        }
    ...
    public function update() {
        ...
        if($this->form_validation->run()) {
            ...
            $query = $this->model_api_key->update($data);
            if ($query) {
                $output['success'] = true;
                $output['message'] = 'DATA BERHASIL DISIMPAN';
            }
    ...
    public function delete($id){
        if($id) {         
            $query = $this->model_api_key->delete($id);
            if ($query) {
                $output['success'] = true;
