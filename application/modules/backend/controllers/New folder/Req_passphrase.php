...
class Req_passphrase extends CI_Controller {

    function __construct(){
        parent::__construct();
        $this->load->model('model_req_passphrase');
        $this->auth->cek_auth('req_passphrase');
    }

    public function index()
    {
        $data['get_instansi'] = $this->model_req_passphrase->get_instansi();
        $this->load->view('req_passphrase/index', $data);
    }

    public function get_satker($id){
        $data['get_satker'] = $this->model_req_passphrase->get_satker($id);
        $this->load->view('req_passphrase/v_filter_satker', $data);
    } 
    ...
    public function list(){
        ...
        $data['total_items'] = $this->model_req_passphrase->list_total($instansi,$status,$like);
        $data['list_items'] = $this->model_req_passphrase->list_data($instansi,$status,$like,$limit,$offset);
        $this->load->view('req_passphrase/v_list', $data );
    }
    ...
    public function reply() {
        ...
        if($this->form_validation->run()) {
            ...
            $query = $this->model_req_passphrase->reply($data);
            if ($query) {
                $output['success'] = true;
    ...
    public function detail($no_ticket){
        if ($no_ticket) {
            $data['detail'] = $this->model_req_passphrase->detail($no_ticket);
            $this->load->view('req_passphrase/v_detail', $data);
        }
        else {
    ...
    public function solved($id){
        if($id) {
            $data['id'] = $id;
            $data['status'] = 'closed'; 
            $query = $this->model_req_passphrase->update($data);
            if ($query) {
                $output['success'] = true;
    ...
    public function delete($id){
        if($id) {         
            $query = $this->model_req_passphrase->delete($id);
            if ($query) {
                $output['success'] = true;
                $output['message'] = 'DATA BERHASIL DIUPDATE';
            }
    ...
    public function response(){
        ...
        $data['list_items'] = $this->model_req_passphrase->list_response($id);
        $this->load->view('req_passphrase/v_response', $data);
    }
