            ...
            <div class="modal-body">
                <form id="form_tambah">
                    <div class="row">
                        ...
                        <div class="col-sm-4">
                            <div class="form-group">
                                <label>Sertifikat P12 </label>
                                <div class="profile-upload">
                                    <div class="upload-img"> <img alt="" src="<?php echo base_url();?>assets/default/img/file-p12.png"></div>
                                    <div class="upload-input">
                                        <input type="file" class="form-control" name="p12" id="p12">
                                    </div>
                                </div>
                            </div>
                        </div>
...
<div class="modal fade" id="modal_upload_p12" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Upload P12</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form_upload_p12">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label>Sertifikat P12</label>
            ...
            <div class="modal-footer text-center">
                <button type="button" class="btn btn-primary submit-btn btn-center" id="btn_upload_p12">Upload</button>
            </div>
        </div>
    </div>
</div>

<div class="modal fade" id="modal_verify_sertifikat" role="dialog">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Verify Sertifikat</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form id="form_verify_sertifikat">
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label>Phasphrase</label>
                                ...
                                <input class="form-control" type="text" name="phasphrase" id="phasphrase">
                            </div>
                        </div>
                    </div>
                </form>
            ...
            <div class="modal-footer text-center">
                <button type="button" class="btn btn-primary submit-btn btn-center" id="btn_verify_sertifikat">Verify</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        tabel = $('#datatable').DataTable({
            ...
            "columns": [
                ...
                { "width": "10%", 
                    "render": function ( data, type, row ) { 
                        if (row.cert_pem == 'ada' && row.cert_p12 == 'ada') {
                            var html  = '<a id="verify_sertifikat" href="#" data-type="pem" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_pem.png" class="rounded-circle m-r-5" alt=""></a><a id="verify_sertifikat" href="#" data-type="p12" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_p12.png" class="roded-circle m-r-5" alt=""></a>'
                        } 
                        else if (row.cert_pem == 'ada' && row.cert_p12 == 'tidak'){
                            var html  = '<a id="verify_sertifikat" href="#" data-type="pem" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_pem.png" class="rounded-circle m-r-5" alt=""></a> <a id="upload_p12" href="#" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_no_p12.png" class="rounded-circle m-r-5" alt=""></a>'
                        }
                        else if (row.cert_pem == 'tidak' && row.cert_p12 == 'ada'){
                            var html  = '<a id="data_detail" href="#" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_no_pem.png" class="rounded-circle m-r-5" alt=""></a> <a id="verify_sertifikat" href="#" data-type="p12" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_p12.png" class="rounded-circle m-r-5" alt=""></a>'
                        }
                        else {
                            var html  = '<a id="data_detail" href="#" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_no_pem.png" class="rounded-circle m-r-5" alt=""></a> <a id="upload_p12" href="#" data-id='+row.nip+'><img width="28" height="28" src="<?php echo base_url();?>assets/default/img/icon_no_p12.png" class="rounded-circle m-r-5" alt=""></a>'
                        }
   ...
   $(document).on("click", "#data_tambah", function() {
    ...
    $('#datatable').on('click','#upload_p12',function(){
        $('#form_upload_p12')[0].reset();
        var nip = $(this).attr("data-id");
        $('#form_upload_p12')[0].reset();
        $('#modal_upload_p12').modal('show');
        $('[name="nip"]').val(nip);

        $('#modal_upload_p12').off('click').on('click','#btn_upload_p12',function(e){
            e.preventDefault();

            var formData = new FormData($('#form_upload_p12')[0]);
    
            $.ajax({
                type: "POST",
                url  : "<?php echo base_url()?>administrator/pengguna/proses_upload",
                data: formData,
                contentType: false,
                processData: false,
                dataType: "JSON",
                success: function(data){
                    $('#modal_upload_p12').modal('hide');
                    reload_table();
                }
            });
            return false;
        ...
        $('#modal_upload_p12').on('hidden.bs.modal', function(e){
           $('#form_upload_p12')[0].reset(); 
        });
    });

    $('#datatable').on('click','#verify_sertifikat',function(){
        $('#form_verify_sertifikat')[0].reset();
        var nip = $(this).attr("data-id");
        var type = $(this).attr("data-type");
        $('#modal_verify_sertifikat').modal('show');
        $('[name="nip"]').val(nip);
        $('[name="type"]').val(type);

        $('#modal_verify_sertifikat').off('click').on('click','#btn_verify_sertifikat',function(e){
            e.preventDefault();
            
            var formData = new FormData($('#form_verify_sertifikat')[0]);
    
            $.ajax({
                type: "POST",
                url  : "<?php echo base_url()?>administrator/pengguna/proses_verify",
                data: formData,
                contentType: false,
                processData: false,
                dataType: "JSON",
                success: function(data){
                    $('#modal_verify_sertifikat').modal('hide');
                    reload_table();
        ...
        $('#modal_verify_p12').on('hidden.bs.modal', function(e){
           $('#form_verify_p12')[0].reset(); 
        });
    });
</script>