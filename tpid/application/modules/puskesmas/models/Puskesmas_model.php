...
class Puskesmas_model extends CI_Model
{
   ...
   function all_by_bentuk_pendidikan()
   {
    ...
    /// puskesmas
    // get all perizinan by month
    function puskesmas_by_bulan($bulan,$tahun)
    {
        ...
        $this->db->where('MONTH(tgl_kunjungan)',$bulan);
        $this->db->where('YEAR(tgl_kunjungan)',$tahun);
        return $this->db->get()->row();
    }

    //get by diagnosa
    function all_diagnosa_filter($bulan,$tahun)
    {
        $this->db->select('diagnosa as label,SUM(jml) as value,baru_l,baru_p,lama_l');
        $this->db->from($this->table_p);
    ...
    //get by puskesmas
    function all_puskesmas_filter($bulan,$tahun)
    {
        $this->db->select('puskesmas as label,sum(jml) as value');
        $this->db->from($this->table_p);
        $this->db->where('MONTH(tgl_kunjungan)',$bulan);
        $this->db->where('YEAR(tgl_kunjungan)',$tahun);
   ...
   function list_tahun()
   {
       $this->db->select('MONTH( tgl_kunjungan ) AS bulan,YEAR ( tgl_kunjungan ) AS tahun');
       $this->db->from($this->table_p);
       $this->db->group_by(array('MONTH(tgl_kunjungan),YEAR(tgl_kunjungan)'));
       return $this->db->get()->result();
   }

   //get all puskesmas
   function all_tabel_puskesmas_filter($bulan,$tahun)
    {
        $this->db->select('MONTH( tgl_kunjungan ) AS bulan,YEAR ( tgl_kunjungan ) AS tahun,puskesmas,sum(jml) as total,SUM(baru_l + baru_p) as kunjungan_baru,SUM(lama_p + lama_l) as kunjungan_lama');
        $this->db->from($this->table_p);
        $this->db->where('MONTH(tgl_kunjungan)',$bulan);
        $this->db->where('YEAR(tgl_kunjungan)',$tahun);
        $this->db->group_by('puskesmas');
   ...
   {
       $this->db->select('MONTH( tgl_kunjungan ) AS bulan,YEAR ( tgl_kunjungan ) AS tahun,puskesmas,kecamatan,sum(jml) as total,SUM(baru_l + baru_p) as kunjungan_baru,SUM(lama_p + lama_l) as kunjungan_lama');
       $this->db->from($this->table_p);
       $this->db->where('MONTH(tgl_kunjungan)',$bulan);
       $this->db->where('YEAR(tgl_kunjungan)',$tahun);
    ...
    {
        $this->db->select('kecamatan,puskesmas,sum(jml) as total,SUM(baru_l + baru_p) as kunjungan_baru,SUM(lama_p + lama_l) as kunjungan_lama');
        $this->db->from($this->table_p);
        $this->db->group_by('kecamatan');
        $this->db->order_by('total','DESC');
        return $this->db->get()->result();
    ...
    function puskesmas_by_kecamatan($kecamatan)
    {
        $this->db->select('puskesmas,sum(jml) as total,SUM(baru_l + baru_p) as kunjungan_baru,SUM(lama_p + lama_l) as kunjungan_lama');
        $this->db->from($this->table_p);
    ...
    function diagnosa_by_puskesmas($puskesmas)
    {
        $this->db->select('diagnosa,sum(jml) as total,SUM(baru_l + baru_p) as kunjungan_baru,SUM(lama_p + lama_l) as kunjungan_lama');
        $this->db->from($this->table_p);
        $this->db->where('puskesmas',$puskesmas);
        $this->db->group_by('diagnosa');
        $this->db->order_by('total','DESC');
        return $this->db->get()->result();
    }
