...
class Sosial extends CI_Controller {
	...
	{
		$data['by_kecamatan'] = $this->Sosial_model->sosial_all_kecamatan();
		$data['layout'] = 'index';
	...
	{
		$get_bpnt = $this->Sosial_model->bpnt_all_kecamatan();
		$get_pkh  = $this->Sosial_model->pkh_all_kecamatan();
		$data = array('bpnt'=>$get_bpnt,
					  'pkh'=>$get_pkh);
	...
	public function modal_sosial_kecamatan()
	{
		header('Content-Type: application/json');
		$kecamatan = $this->input->get('kecamatan');
		$bpnt_filter_kecamatan = $this->Sosial_model->bpnt_filter_kecamatan($kecamatan);
		$pkh_filter_kecamatan = $this->Sosial_model->pkh_filter_kecamatan($kecamatan);
		
		$data = array('bpnt_filter' => $bpnt_filter_kecamatan,
					 'pkh_filter' => $pkh_filter_kecamatan);
		echo json_encode($data);
	}
	...
	public function modal_evb()
	{
		...
		$by_desa    = $this->Sosial_model->by_desa_filter($desa);
	...
	public function table_filter()
	{
		...
		if($bulan == 'semua') {
			$by_kec_filter = $this->Sosial_model->by_kec_all();
			$data_res = '';
			...
			$by_kec_filter = $this->Sosial_model->by_kec_filter($bulan,$tahun);
			$data_res = '';
				foreach($by_kec_filter as $data) {
